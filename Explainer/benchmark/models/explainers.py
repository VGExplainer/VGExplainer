...
class ExplainerBase(nn.Module):
    ...
    def control_sparsity_sum(self, edge_index, edge_mask, name, sparsity=None): ###sum pooling

        node_scores = {} 


        for i in range(edge_index.shape[1]):
            source_node_id = int(edge_index[0, i].item())
            target_node_id = int(edge_index[1, i].item())
            mask_value = float(edge_mask[i].item())

            node_scores[source_node_id] = node_scores.get(source_node_id, 0.0) + mask_value
            node_scores[target_node_id] = node_scores.get(target_node_id, 0.0) + mask_value
        ...
        sorted_node = [node_id for node_id, score in sorted_nodes_with_scores]

        
        node_dict = {}
        for rank, node_id in enumerate(sorted_node):
            node_dict[node_id] = rank

        return sorted_node, sorted_node, node_dict
    
    def control_sparsity_mean(self, edge_index, edge_mask, name, sparsity=None): ###mean pooling

        
        node_accumulators = defaultdict(lambda: {'sum': 0.0, 'count': 0})
        ...
        for i in range(edge_index.shape[1]):
            ...
            node_accumulators[source_node_id]['sum'] += mask_value
            node_accumulators[source_node_id]['count'] += 1

           
            node_accumulators[target_node_id]['sum'] += mask_value
            node_accumulators[target_node_id]['count'] += 1
            
        
        node_mean_scores = {}
        if not node_accumulators: 
            pass
        else:
            for node_id, data in node_accumulators.items():
                if data['count'] > 0:
                    node_mean_scores[node_id] = data['sum'] / data['count']
                else:
                    node_mean_scores[node_id] = 0.0 

        
        sorted_nodes_with_mean_scores = sorted(
            node_mean_scores.items(),
            key=lambda item: item[1], 
            reverse=True 
        )
        sorted_node = [node_id for node_id, score in sorted_nodes_with_mean_scores]
...
#     def control_sparsity_sum(self, edge_index, edge_mask, name, sparsity=None):
#         r"""
...
#         node_dict = {}
#         for rank, node_id in enumerate(sorted_indices):
#             node_dict[node_id] = rank
#         return sorted_indices,sorted_indices,node_dict
    

    def control_sparsity(self, edge_index, edge_mask, name, sparsity=None):### max pooling 
...
#
        ...
        for indice in sorted_indices:
            ...
            number += 1
#
        out_path = '/home/DIG-main/dig/xgraph/' + name[0]
    ...
    def visualize_graph(self, node_idx, edge_index, edge_mask, y=None, name =None,
        ...
        with open('/gnn_results/' + name[0], 'w') as wp:
...
class MyVulExplainer(ExplainerBase):
    ...
    def forward(self, data, mask_features=False,
        ...
        if int(predicted) == 0:
            print("predict = 0")
            return
        dot_name = '/home/Dataset/primevul/3_export/pdg_norm/1_vul/'+name.split('.json')[0]+'.dot' ####
        nx_pdg = nx.nx_agraph.read_dot(dot_name)
        if type(nx_pdg) != None:
        ...
        for i, ex_label in enumerate(ex_labels):
            ...
            # sorted_node_in, sorted_node_out, node_dict = self.control_sparsity_sum(slice_edge_index, self.gnn_explainer_alg(slice_x, slice_edge_index, edge_attr, ex_label), name)
            ...
            # sorted_node_list_func = []
            # for node_item in sorted_node_list:
            #     sorted_node_list_func.append(node_index[node_item])

            # exp_name = name.split('.json')[0]+'###expfunc'+'.json'
            # exp_path = '/home/VulGnnExp/rq2/results/deepwukong/5/'+ name.split('.json')[0]+'/'
            # # with open(exp_path,'w') as funcf:
            # #     json.dump(sorted_node_list[:sparsity_num],funcf)
            # if os.path.exists(exp_path):
            #     exp_write_path = exp_path + exp_name
            #     with open(exp_write_path,'w') as wf:
            #         json.dump(sorted_node_list_func[:sparsity_num],wf)
            # else:
            #     os.mkdir(exp_path)
            #     exp_write_path = exp_path + exp_name
            #     with open(exp_write_path,'w') as wf:
            ...
            if func_res_coalition!=[]:
                for coalition_item in func_res_coalition:
                    ...
                    exclude_score = exclude_preds[:, 1].detach()
                    # exp_dict[tuple(coalition_item)] = func_score-exclude_score
                    exp_dict[tuple(coalition_item)] = (func_score - exclude_score).cpu().item()
                    del exclude_xi, exclude_edgeindexi, exclude_preds, node_mask
            ...
            for index,exp_item in enumerate(res_coalition):
                exp_name = name.split('.json')[0]+'###expfunc'+'.json'
                exp_path = '/home/VGExplainer/res_primevul/raw_res/reveal/1_vul/5/'+ name.split('.json')[0]+'/' ###
                if os.path.exists(exp_path):
                    exp_write_path = exp_path + exp_name
                    with open(exp_write_path,'w') as wf:
        ...
        res_coalition = []
        slice_path = '/home/VGExplainer/slice/1_vul/'+name.split('.json')[0] ###
        ...
        if slice_lists != []:
            ...
            for slice_item in tqdm(slice_lists):
                slice_nodes=[]
                try:
                    slice_pdg = nx.nx_agraph.read_dot(slice_item)
                except:
                    continue
                ...
                with torch.no_grad():
                    slice_preds_full = self.model(slice_xi, edge_index)
                    _, slice_predict = slice_preds_full.max(1)
                    slice_preds = self.model(slice_xi, slice_edgeindexi)
                # slice_score = slice_preds[:, 1]
                slice_score = slice_preds[:, 1].detach().cpu().item()
                if(slice_predict == 1 and len(slice_nodes)>sparsity_num):
                    # slice_dict[tuple([slice_xi,slice_edgeindexi,slice_pdg])] = slice_score
                    slice_dict[tuple([slice_xi.cpu(), slice_edgeindexi.cpu(), slice_pdg])] = slice_score
                ...
                del slice_xi, slice_edgeindexi, slice_preds_full, slice_preds
            ...
            if slice_dict != {}:
                ...
                for slice_item in slice_dict:
                    ...
                    keys_list = slice_item
                    first_key = keys_list[0]
                    slice_x = first_key[0].to(x.device) 
                    slice_edge_index = first_key[1].to(x.device)
                    slice_pdg = first_key[2]
                    ...
                    # sorted_node_list_slice=[]
                    # for node_item in sorted_node_list:
                    #     sorted_node_list_slice.append(node_index[node_item])
                    # res_coalition.append(sorted_node_list_slice[:sparsity_num])########
                        ...
                        if slice_res_coalition!=[]:
                            for coalition_item in slice_res_coalition:
                                ...
                                exclude_score = exclude_preds[:, 1].detach()
                                exp_dict[tuple(coalition_item)] = (slice_score-exclude_score).cpu().item()
                                del exclude_xi, exclude_edgeindexi, exclude_preds, node_mask
                            exp_dict = sorted(exp_dict.items(),key=lambda x:x[1],reverse=True)
            ...
            #                 json.dump(exp_item,wf)
                ...
                for index,exp_item in enumerate(res_coalition): 
                    ...
                    slice_score_dict[tuple(exp_item)] = (func_score - exclude_score).cpu().item()
                    del exclude_xi, exclude_edgeindexi, exclude_preds, node_mask
...
class CFExplainer(ExplainerBase):
    ...
    def __init__(self,
                 ...
                 L1_dist: bool = False):
        super(CFExplainer, self).__init__(model, epochs, lr, explain_graph, molecule)
        self.alpha = alpha
...
#         return sorted_node_in, sorted_node_out, node_dict
    ...
    def control_sparsity(self, edge_index, edge_mask, name, sparsity,node_index):
        r"""
...
# #
        ...
        #     if int(edge_index[1][i]) not in node_dict:
        #         node_dict[int(edge_index[1][i])] = index
        # sorted_nodes = sorted(node_dict.keys(), key=lambda x: node_dict[x])

        # # 2. 计算需要保留的节点数量 (1 - sparsity%)
        # # 注意：sparsity 通常指剔除比例，1 - sparsity 则是保留比例
        # keep_ratio = 1 - sparsity
        # num_important = max(1, int(len(sorted_nodes) * keep_ratio)) # 确保至少保留一个节点

        # # 3. 截取排在前面的重要节点
        # important_indices = sorted_nodes[:num_important]
        important_nodes = []
        for important_indice in important_indices:
            important_nodes.append(node_index[important_indice])
#
        out_path = '/home/CFExplainer/res_primevul/raw_res/reveal/' + name   ###
        with open(out_path, 'w') as wp:
            #wp.write('node length: ' + str(x_len) + ' \n')
            #wp.write('important node length: ' + str(len(important_indices)) + ' \n')
            json.dump(important_nodes, wp)
    ...
    def __set_masks__2(self, x, edge_index, init="normal"):
        (N, F), E = x.size(), edge_index.size(1)
    ...
    def __loss__(self, raw_preds: Tensor, x_label: Union[Tensor, int]):
        relu = torch.nn.ReLU()
        if self.explain_graph:
            loss = relu(raw_preds[:, int(x_label)]).sum()
        else:
            loss = relu(raw_preds[self.node_idx, int(x_label)]).sum()
    ...
    def forward(self, data, mask_features=False, target_label=None, **kwargs):
        r"""
        ...
        for i, ex_label in enumerate(ex_labels):
            ...
            try:
                ...
                continue
            self.control_sparsity(self_loop_edge_index, self.gnn_explainer_alg(x, edge_index, edge_attr, ex_label),name = name, sparsity=sparsity,node_index=node_index)
            # sorted_node_in, sorted_node_out, node_dict = self.control_sparsity(self_loop_edge_index, self.gnn_explainer_alg(x, edge_index, edge_attr, ex_label), name, sparsity=0.8)
            # if len(sorted_node_in) > len(sorted_node_out):
            #     sorted_node_list = sorted_node_in
            # else:
            #     sorted_node_list = sorted_node_out
            ...
            #     sorted_node_list_func.append(node_index[node_item])
            # exp_name = name
            # exp_path = '/home/VulGnnExp/rq1/results/CFExp/devign/'+exp_name
            # with open(exp_path,'w') as funcf:
            #     json.dump(sorted_node_list_func[:sparsity_num],funcf)
...
class CocaExp(ExplainerBase):
    ...
    def __init__(self,
                 ...
                 indirect_graph_symmetric_weights: bool = False,
                 L1_dist: bool = False):
        super(CocaExp, self).__init__(model, epochs, lr, explain_graph, molecule)
        self.alpha = alpha
        self.L1_dist = L1_dist
        self.gam = 1
        self.lam = 0.1
...
# #
        ...
        for important_indice in important_indices:
            important_nodes.append(node_index[important_indice])
#
        out_path = '/home/Coca/res_primevul/raw_res/reveal/' + name
        with open(out_path, 'w') as wp:
            #wp.write('node length: ' + str(x_len) + ' \n')
            #wp.write('important node length: ' + str(len(important_indices)) + ' \n')
            json.dump(important_nodes, wp)
    ...
    def __set_masks__(self, x, edge_index, init="normal"):
        ...
        try:
            ...
            return 'error'
        # mask = torch.nn.Parameter(torch.FloatTensor(x.shape[0], x.shape[0]))
        # std = torch.nn.init.calculate_gain("relu") * math.sqrt(
        #     2.0 / (x.shape[0] + x.shape[0])
        # )
        # with torch.no_grad():
    ...
    def lossfunc(self, pred1, pred2, pred_label, masked_adj, gam, lam, alp):
        relu = torch.nn.ReLU()
    ...
    def gnn_explainer_alg2(self,
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if mask_features:
                ...
                h = x
            try:
                pred1 = self.model(h, edge_index)[0, 1]
            except:
                return None
            model2 = self.model
            model2 = model2.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
            for i, module in enumerate(model2.modules()):
                if isinstance(module, MessagePassing):
                    module.__explain__ = True
                    masked_adj = module.__edge_mask__
                    module.__edge_mask__.data = -module.__edge_mask__.data
            pred2 = model2(h, edge_index)[0, 0]
            loss = self.lossfunc(pred1,pred2,ex_label,masked_adj,self.gam,self.lam,self.alp)
            #loss = loss_function(raw_preds, ex_label)
            if epoch % 20 == 0:
                print(f'#D#Loss:{loss.item()}')
    ...
    def forward(self, data, mask_features=False, target_label=None, **kwargs):
        ...
        for i, ex_label in enumerate(ex_labels):
            if i == 0:
                continue
            self.__clear_masks__()
            test_code = self.__set_masks__(x, self_loop_edge_index)
            if test_code == 'error':
                return 
            #self.control_sparsity(self.gnn_explainer_alg(x, edge_index, edge_attr, ex_label), sparsity=kwargs.get('sparsity'))
            self.control_sparsity(self_loop_edge_index, self.gnn_explainer_alg2(x, edge_index, edge_attr, ex_label),name = name, sparsity=sparsity,node_index=node_index)
            # sorted_node_in, sorted_node_out, node_dict = self.control_sparsity(self_loop_edge_index, self.gnn_explainer_alg2(x, edge_index, edge_attr, ex_label), name, sparsity=0.8)
            # if len(sorted_node_in) > len(sorted_node_out):
            #     sorted_node_list = sorted_node_in
            # else:
            #     sorted_node_list = sorted_node_out
            ...
            #     sorted_node_list_func.append(node_index[node_item])
            # exp_name = name
            # exp_path = '/home/VulGnnExp/MyVulExplainer/results/cfexplaienr/ivdetect/'+exp_name
            # with open(exp_path,'w') as funcf:
            #     json.dump(sorted_node_list_func[:sparsity_num],funcf)
...
# #
        ...
        for important_indice in important_indices:
            important_nodes.append(node_index[important_indice])
#
        out_path = '/home/GNNExplainer/res_primevul/raw_res/reveal/' + name
        with open(out_path, 'w') as wp:
            #wp.write('node length: ' + str(x_len) + ' \n')
            #wp.write('important node length: ' + str(len(important_indices)) + ' \n')
            json.dump(important_nodes, wp)
