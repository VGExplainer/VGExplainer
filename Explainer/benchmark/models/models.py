...
class DevignModel(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        graph_embedding = global_mean_pool(concatenated_features, torch.zeros(concatenated_features.shape[0], dtype=torch.long, device=concatenated_features.device))
...
class IVDetect(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        pooled = global_max_pool(post_conv, torch.zeros(post_conv.shape[0], dtype=torch.long, device=post_conv.device))
        return F.softmax(pooled, dim=-1)

class RevealModel(nn.Module):
    def __init__(self, hidden_dim=256, num_classes=2, num_steps=4, dropout_rate=0.2):
        super().__init__()
    ...
    def forward(self, x, edge_index):
        ...
        graph_embedding = global_add_pool(node_embeddings, torch.zeros(node_embeddings.shape[0], dtype=torch.long, device=x.device))
...
class DeepWukongModel(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        initial_batch = torch.zeros(node_embedding.shape[0], dtype=torch.long, device=x.device)
