...
class XCollector(object):
    ...
    def collect_data(self,
        ...
        if not np.isnan(label): #is not a none
...
def sample_explain(explainer, data, x_collector: XCollector, **kwargs):
    ...
    if torch.isnan(data.y[y_idx].squeeze()):
        return
    if os.path.exists('/home/mytest/GNNExplainer-master/gnn_results/'+ data.name[0]):
        print(data.name[0]+' has been precessed')
    else:   
        explain_tik = time.time()
        explainer(data, **kwargs) #related_preds? explainer -> GNNExplainer -> forward 
        explain_tok = time.time()
        print(f"#D#Explainer prediction time: {explain_tok - explain_tik:.4f}")
    #masks, related_preds都是两组数值，为0和1标签分别生成的
    ...
    if False:
        ...
        if x_args.walk:
            ...
            explainer.visualize_graph(node_idx=0 if node_idx is None else node_idx, edge_index=data.edge_index,
                                             edge_mask=masks[gt_label],
                                             y=data.x[:, 0] if node_idx is None else data.y, name = data.name, num_nodes=data.x.shape[0])
