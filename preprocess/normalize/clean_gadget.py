...
keywords = frozenset(['__asm', '__builtin', '__cdecl', '__declspec', '__except', '__export', '__far16', '__far32',
                      ...
                      'size_t', 'ssize_t', 'loff_t', 'uint8_t', 'uint16_t', 
...
KERNEL_ATTRIBUTES = r'\b__(user|init|rcu|kernel|safe|init|exit|read_mostly|aligned|packed)\b'

def clean_gadget(gadget_lines):
    full_code = "".join(gadget_lines)

    # --- Step 1 & 2: Initial Cleaning ---
    # Strip branch prediction hints
    full_code = re.sub(r'\b(unlikely|likely)\s*\((.*)\)', r'(\2)', full_code)
    # Strip kernel-specific attributes
    full_code = re.sub(KERNEL_ATTRIBUTES, '', full_code)
    # Remove non-ASCII characters
    full_code = re.sub(r'[^\x00-\x7f]', r'', full_code)
    # Abstract strings and characters
    full_code = re.sub(r'".*?"', '"S"', full_code)
    full_code = re.sub(r"'.*?'", "'C'", full_code)

    # --- Step 3: Enhanced Identifier Classification and Extraction ---
    
    # 3.1 Explicit type tags (struct/enum/union)
    type_tags = set(re.findall(r'\b(?:struct|enum|union)\s+([_A-Za-z]\w*)\b', full_code))
    
    # 3.2 Heuristic Custom Type Recognition
    # Pattern A: Words following typedef -> e.g., typedef int MyType;
    typedefs = re.findall(r'\btypedef\s+(?:[_A-Za-z]\w*\s+)+([_A-Za-z]\w*)\b\s*;', full_code)
    type_tags.update(typedefs)
    
    # Pattern B: Typical variable declaration structures -> e.g., MyType VAR_1; or MyType *VAR_2;
    # If a word is followed by another word or a pointer symbol + word, and isn't a keyword, it's likely a type.
    declaration_types = re.findall(r'\b([_A-Za-z]\w*)\b\s*(?:\*+\s*)?\b[_A-Za-z]\w*\b\s*[;=,)]', full_code)
    for t in declaration_types:
        if t not in keywords and t not in ['return', 'else', 'goto']:
            type_tags.add(t)

    # 3.3 Extract function names (identifiers followed by an open parenthesis)
    fun_candidates = set(re.findall(r'\b([_A-Za-z]\w*)\b\s*\(', full_code))
    
    # 3.4 Extract all words for mapping
    all_words = set(re.findall(r'\b([_A-Za-z]\w*)\b', full_code))
    ...
    for name in all_words:
        ...
        # If recognized as a type, use TAG_ prefix
        if name in type_tags:
            if name not in mapping:
                mapping[name] = f'TAG_{tag_count}'
                tag_count += 1
        # If recognized as a function call/definition, use FUN_ prefix
        elif name in fun_candidates:
            if name not in mapping:
                mapping[name] = f'FUN_{fun_count}'
                fun_count += 1
        # Default to variable prefix VAR_
        else:
            if name not in mapping:
                mapping[name] = f'VAR_{var_count}'
                var_count += 1

    # --- Step 5: Batch Regex Substitution ---
    if not mapping:
        return [full_code]

    # Sort by length descending to prevent sub-string replacement errors (e.g., 'var1' replacing 'var11')
    pattern = re.compile(r'\b(' + '|'.join(re.escape(k) for k in sorted(mapping.keys(), key=len, reverse=True)) + r')\b')
    
    # Execute replacement via callback
    final_code = pattern.sub(lambda x: mapping[x.group(0)], full_code)
